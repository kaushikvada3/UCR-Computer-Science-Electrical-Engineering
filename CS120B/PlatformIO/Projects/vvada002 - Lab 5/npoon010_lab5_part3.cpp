 ...
 *         Demo Link: https://www.youtube.com/watch?v=1dqfANhKXYo
 */
#include "timerISR.h"
#include "helper.h"
#include "periph.h"
...
int tracked_distance = 10;
int threshold_close = 8;
int threshold_far = 12;
unsigned char update_in_progress = 0;
unsigned char blue_blink_state = 0;
int blue_blink_counter = 0;
...
int RB_Tick(int state) {
  switch(state) {
    ...
    case RB_wait:
      if (PINC >> 1 & 0x01) { // Right button pressed
    ...
    case RB_blink:
      if (blue_blink_counter >= 12) { // 250ms * 12 = 3000ms
...
int BL_Tick(int state) {
  switch(state) {
    ...
    case BL_toggle:
      if (!update_in_progress) {
        PORTC &= 0b11111011; // Blue off
        state = BL_wait;
      } else {
        blue_blink_state ^= 1;
        if (blue_blink_state) {
          PORTC |= 0b00100000; // Blue ON
        } else {
          PORTC &= 0b11011111; // Blue OFF
        }
        blue_blink_counter++;
      }
      break;
