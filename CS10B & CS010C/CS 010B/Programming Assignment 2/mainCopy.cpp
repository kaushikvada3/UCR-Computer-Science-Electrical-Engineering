...
Date::Date() { //default constructor with default values
   this->day = 1;
   month = 1;
   monthName = "January";
   year = 2000;
...
Date::Date(unsigned m, unsigned d, unsigned y) {
    if (m < 1 || m > 12)
   {
    ...
    {
        month = m;
        unsigned maxNumDays = daysPerMonth(m, y);
        if (d < 1 || d > maxNumDays) {
            cout << "Invalid date values: Date corrected to " << m << "/" << maxNumDays << "/" << y << "." << endl;
            day = maxNumDays;
        } else {
            day = d;
        }
...
Date::Date(const string &mn, unsigned d, unsigned y) {
    ...
    if (month == 100) {
        ...
        unsigned maxNumDays = daysPerMonth(month, y);
        if (d < 1 || d > maxNumDays) {
            cout << "Invalid date values: Date corrected to " << month << "/" << maxNumDays << "/" << y << "." << endl;
            day = maxNumDays;
        } else {
            day = d;
        }
...
string Date::name(unsigned m) const {
    static const string namesOfMonths[] = {
