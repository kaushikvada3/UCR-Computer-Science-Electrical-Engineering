...
{
    clearNodesMemory(root); //using the helper function to recursively clear the nodes. 
}

void arithmeticExpression::clearNodesMemory(TreeNode *node)
{
    if (node == nullptr) {
        return; // Base case: if node is null, do nothing
    }
    
    clear(node->left);  // First, delete left subtree
    clear(node->right); // Then, delete right subtree
...
arithmeticExpression::arithmeticExpression(const string &value) {
    ...
    infixExpression = value; // sets infix expression to the value passed in
}
...
// outputs tree in in order notation
void arithmeticExpression::infix(TreeNode *input) {
    ...
    if (priority(input->data) != 0) { // check for priority -- if there's priority, call recursively until leaf, and add parenthesis
        ...
        infix(input->left); // call helper function on left child
        cout << input->data; // print current node
        infix(input->right); // call helper function on right child
