...
{
	size = s;
	hashTable = new list<WordEntry>[s]; // initializes list with size s
}
...
int HashTable::computeHash(const string &s)
{
	...
	{
		hashNum += (int)s[i]; // sum of the characters in string s
	}

	return (hashNum * 2 + size - 31) % size; // get the hash number based on the algorithm given the number of charcters in s
}
...
{
	int index = computeHash(s); // use helper function to get hash value (index)
	if (contains(s)) // only do loop if word exists
	{
		list<WordEntry>::iterator it; // use iterator to loop through the list
		it = hashTable[index].begin();
		while (it != hashTable[index].end())
		{
			if (it->getWord() == s) // use add new appearance once word is found
			{
	...
	hashTable[index].push_back(WordEntry(s, score)); // if word is not in hash table, add new word at the end
}
...
{
	if (contains(s)) // if word exists, we can return an average
	{
		int index = computeHash(s); // same as put function, but gets the average of value using word entry get average function
...
{
	int index = computeHash(s); // helper function that loops through using an iterator to find if a word exist 
	list<WordEntry>::iterator it;
	it = hashTable[index].begin();
	while (it != hashTable[index].end())
	{
